...
import kamofoke.swingy.model.Villians.Villian;
//import kamofoke.swingy.model.Villians.*;
...
public class GuiMap extends JFrame {
    ...
    public void hasWon() {
        ...
        if (this.level > hero.getHeroStats().getLevel()) {
            ...
            JOptionPane.showMessageDialog(null, "Level Done! You can now move to the next level.");
    ...
    public boolean crossedEnemy(int hy, int hx, int ey, int ex) {
        if ((hx == ex) && (hy == ey)) {
            ...
            int dialogResult = JOptionPane.showConfirmDialog(this, "You've met Enemy, Do you wana fight them?", "Be a chicken or Fight like a hero?", dialogButton);
            if (dialogResult == 0) {
                if (fight() == 1) {
                    return true;
                } else {
    ...
    public int fight() {
        ...
        if (hero.getHeroStats().getHitPoints() > 0) {
            ...
            JOptionPane.showMessageDialog(null, "Need training, I suggest you go home and do some training, In order to continue fighting");
        return won;
    }
}
